...
create table stage_sch.location (
    ...
    createddate text,
    modifieddate text,
    -- audit columns for trackiing & debugging
    _stg_file_name text,
    _stg_file_load_ts timestamp,
    _stg_file_md5 text,
    _copy_data_ts timestamp default current_timestamp
)
comment = 'This is the location stage/raw table where data will be copied from internal stage using copy command. This as-is data
representation from the source location. All the columns are text data type except the audit columns that are added for traceability.';
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_sch.csv_stg/delta/location/delta-day01-2rows-insert.csv t  -- load day 2 as well
)
file_format = (format_name = 'stage_sch.csv_file_format')
on_error = abort_statement;
